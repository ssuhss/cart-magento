...
(function () {
    ...
    var MPv1 = {
        ...
        selectors: {
            ...
            cardTruncated: "#mp_cardTruncated",
            site_id: "#mp_site_id",
            CustomerAndCard: '#mp_customerAndCard',
            ...
            //@gateway
            gateway_mode: "#mp-mercadopago-gateway-mode"
    ...
    MPv1.AJAX = function (options) {
        let useXDomain = !!window.XDomainRequest;
        let req = useXDomain ? new XDomainRequest() : new XMLHttpRequest()
        let data;
        let referer = window.location.protocol + "//" + window.location.hostname + (window.location.port ? ':' + window.location.port : '');

        options.url += (options.url.indexOf("?") >= 0 ? "&" : "?") + "referer=" + escape(referer);
    ...
    MPv1.guessingPaymentMethod = function (event) {
        ...
        if (event.type == "keyup") {
            if (bin != null && bin.length == 6) {
                Mercadopago.getPaymentMethod({"bin": bin}, MPv1.setPaymentMethodInfo);
            }
        } else {
            setTimeout(function () {
                if (bin.length >= 6) {
